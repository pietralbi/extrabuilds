...
{
	{
		...
		growsound   = "extrabuilds/marbleshrub/wilt_to_grow",
	...
	},
	{
		...
		growsound   = "extrabuilds/marbleshrub/grow",
		workleft    = TUNING.MARBLESHRUB_MINE_NORMAL,
   		loot = { "marble" },
    	chanceloot = {
        	{ prefab = "marblebean", chance = 0.25 },
    	},
	},
	{
        name		= "tall",
		...
		loot = { "marble" },
    	randomloot = {
			{ prefab = "marble",     weight = 1 },
			{ prefab = "marblebean", weight = 1 },
		},
    	numrandomloot = 1,
	},
}

local function ApplyStageLoot(inst)
    local ld = inst.components.lootdropper
    ld:SetLoot(inst.statedata.loot or nil)

    -- Chance loot
    ld.chanceloot = nil
    if inst.statedata.chanceloot then
        for _, v in ipairs(inst.statedata.chanceloot) do
            ld:AddChanceLoot(v.prefab, v.chance)
        end
    end

    -- Random loot
    ld.randomloot = nil
    ld.totalrandomweight = nil
    ld.numrandomloot = nil
    if inst.statedata.randomloot and inst.statedata.numrandomloot then
        ld.numrandomloot = inst.statedata.numrandomloot
        for _, v in ipairs(inst.statedata.randomloot) do
            ld:AddRandomLoot(v.prefab, v.weight)
        end
    end
end
...
local function SetupShrubShape(inst, buildnum)
	...
	inst.MiniMapEntity:SetIcon("marbleshrub"..tostring(inst.shapenumber)..".tex")
...
local function GrowFromSeed(inst)
    ...
    inst.SoundEmitter:PlaySound("extrabuilds/marbleshrub/grow")
end
...
local function MakeShrub(name, growthstage)
	...
	local function fn()
        ...
        inst.components.growable.loopstages = TUNING.MARBLESHRUB_LOOPING
